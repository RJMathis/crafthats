...
class Test_API_Style(unittest.TestCase):
	...
	def testStyleBasic(self):
		response = self.app.get('/styles').data		 
		response = json.loads(response)
		self.assertEqual(len(response['records']),2)
	
	def testPagination(self):
		response = self.app.get('/styles?limit=1').data		 
		response = json.loads(response)
		self.assertEqual(len(response['records']),1)
		response = self.app.get('/styles?offset=1').data		 
		response = json.loads(response)
		self.assertEqual(len(response['records']),1)
		self.assertEqual(response['records'][0]['name'],'style2')
	
	def testAbvMinMaxParam(self):
		response = self.app.get('/styles?abv_min=4.0&abv_max=9.0').data		 
		response = json.loads(response)
		self.assertEqual(len(response['records']),1)
		self.assertEqual(response['records'][0]['name'],'style1')
		
	def testIbuMinMaxParam(self):
		response = self.app.get('/styles?srm_min=1.0&srm_max=11.0').data		 
		response = json.loads(response)
		self.assertEqual(len(response['records']),1)
		self.assertEqual(response['records'][0]['name'],'style1')
		response = self.app.get('/styles?srm_min=1.0&srm_max=20.0').data		 
		response = json.loads(response)
		self.assertEqual(len(response['records']),2)

	
	def testSortByAsc(self):
		response = self.app.get('/styles?sort_by=asc').data		 
		response = json.loads(response)
		self.assertEqual(response['records'][0]['name'],'style1')

		
	def testSortByDesc(self):
		response = self.app.get('/styles?sort_by=desc').data		 
		response = json.loads(response)
		self.assertEqual(response['records'][0]['name'],'style2')
		

	def testAbvAndSrm(self):
		response = self.app.get('/styles?abv_min=4.0&abv_max=9.0&srm_min=1.0&srm_max=9.0').data		 
		response = json.loads(response)
		self.assertEqual(len(response['records']),0)
		response = self.app.get('/styles?abv_min=4.0&abv_max=9.0&srm_min=1.0&srm_max=10.0').data		 
		response = json.loads(response)
		self.assertEqual(len(response['records']),1)
		self.assertEqual(response['records'][0]['name'],'style1')
	...
	def testStyleById(self):
		response = self.app.get('/styles/1').data
		response = json.loads(response)
		self.assertEqual(response['name'],'style1')	
		
		response = self.app.get('/styles/2').data
		response = json.loads(response)
		self.assertEqual(response['name'],'style2')
		
		response = self.app.get('/styles/100').data
		self.assertEqual(response,'Server Error 500: Invalid style_id')	
